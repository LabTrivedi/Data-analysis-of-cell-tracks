...
save_directory = "//trivedi.embl.es/trivedi/Kerim_Anlas/for_jordi/all_tracking_data_plots/Arrow_at_origin/"
printInFolder = True
mintrac = 60
        ...
        for Cat in Cats:
            ...
            for cell in range(max_cell+1):
                ...
                if len(time) >= mintrac:
                    ...
                    time = np.where(csv[:, 1] == cell)[0]
                    if Cat == 0:
                        max_range = max(abs((csv[time[-1], 4] - csv[time[0], 4])/len(time)),
                                        abs((csv[time[-1], 5] - csv[time[0], 5])/len(time)),
                                        max_range)
                    if Cat != cat:
                        ax[xx, yy].quiver(0.0, 0.0, (csv[time[-1], 4] - csv[time[0], 4])/len(time), (csv[time[-1], 5] - csv[time[0], 5])/len(time), color='black', alpha=0.25, angles='xy', scale_units='xy', scale=1.0)
                    else:
                        ax[xx, yy].quiver(0.0, 0.0, (csv[time[-1], 4] - csv[time[0], 4])/len(time), (csv[time[-1], 5] - csv[time[0], 5])/len(time), color=colors[Cat], angles='xy', scale_units='xy', scale=1.0)
            if Cat == 0:
                max_range *= 1.1
            ax[xx, yy].set_xlim(-max_range, max_range)
            ax[xx, yy].set_ylim(-max_range, max_range)
        ax[0, 0].axis('off')
